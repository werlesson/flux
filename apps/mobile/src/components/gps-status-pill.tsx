...
const labels: Record<GpsStatus, string> = { good: 'GPS: boa precisão', degraded: 'GPS: precisão degradada', 'no-signal': 'GPS: sem sinal', unacceptable: 'GPS: sem precisão aceitável' };
export function GpsStatusPill({ status }: { status: GpsStatus }) {
  const theme = useTheme();
  const color = status === 'good' ? theme.colors.confirmation : status === 'degraded' ? theme.colors.highlight : theme.colors.destructive;
