...
export class ActivitiesRepository {
  ...
  async atualizarMetricas(id: number, metrics: ActivityMetrics, at = new Date()): Promise<void> {
    ...
    const hasBestPace = metrics.best_pace_seconds_per_km !== undefined;
    const result = await this.database.run(`UPDATE activities SET elapsed_duration_seconds=?,moving_duration_seconds=?,distance_meters=?,average_pace_seconds_per_km=?,best_pace_seconds_per_km=CASE WHEN ?=1 THEN ? ELSE best_pace_seconds_per_km END,finished_at=COALESCE(?,finished_at),activity_status_id=COALESCE(?,activity_status_id),updated_at=? WHERE id=? AND finished_at IS NULL`, [metrics.elapsed_duration_seconds, metrics.moving_duration_seconds, metrics.distance_meters, metrics.average_pace_seconds_per_km ?? null, hasBestPace ? 1 : 0, metrics.best_pace_seconds_per_km ?? null, metrics.finished_at ? now(metrics.finished_at) : null, statusId, now(at), id]);
